...
const ProofViewerModal = ({ isOpen, onClose, proofData }) => {
  ...
  const { inputs, proof: cryptoProof } = proof;
  const timestamp = inputs?.timestamp ? new Date(Number(inputs.timestamp)).toLocaleString() : "Just now";
      ...
      <DialogContent sx={{ textAlign: 'center', p: 4 }}>
        ...
        <CardSection>
            ...
            <InfoRow 
                ...
                value={inputs?.ageCheck === "OVER_18" ? "18+ CONFIRMED" : "FAILED"} 
                isSuccess={inputs?.ageCheck === "OVER_18"}
            />
            <Divider sx={{ my: 1.5 }} />
            <InfoRow 
